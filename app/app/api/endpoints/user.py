...
from app.api.dependencies import RegisterUser, LoginUser, OffsetLimitParam, CurrentUser
...
async def register(request: Request, registred_user: RegisterUser):
    return dict(detail=[registred_user], info_api=request)
...
async def get_one(
    ...
    return dict(detail=[current_user], info_api=request)


@router.get(
    "/get-all/", response_model=ResponseWithParams[Union[ResponseUserSchema, dict]]
)
async def get_all(request: Request, params: OffsetLimitParam, uow_context: UOWV1Dep):
    ...
    return dict(detail=response, info_api=request, params=params)
