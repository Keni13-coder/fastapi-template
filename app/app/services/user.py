...
class ABCUserService(abc.ABC):
    ...
    @abc.abstractmethod
    async def register_user(self, user: CreateUser, uow_context: UOWV1Dep) -> ResponseUserSchema:
        '''Метод вызываемый в поинтах, для регистрации пользователя в базе'''
        raise NotImplementedError
    
    @abc.abstractmethod
    async def login_user(self, login_data: Annotated[LoginUser, Depends(FormAuth)], uow_context: UOWV1Dep) -> TokenLoginResponse:
...
class UserService(ABCUserService):
    ...
    async def login_user(
        ...
        async with uow_context as uow:
            ...
            tokens: ResponseToken = await self._token_service.create_pair_tokens(user_uid=user.id, uow_context=uow_context)
            response.set_cookie(key='token', value=tokens.refresh_token, expires=tokens.expire_refresh, httponly=True, secure=True)
