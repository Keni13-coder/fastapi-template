...
class TestContextUser:
    ...
    async def test_update_user_login(
        ...
        async with get_fakeUOW as uow:
            user = await uow.user.update(
                id=self.user_data["id"], obj_in=update_data_user_login
            )
            await uow.commit()
    ...
    async def test_update_user_password(
        ...
        async with get_fakeUOW as uow:
            user = await uow.user.update(
                id=self.user_data["id"], obj_in=update_data_user_password
            )
            await uow.commit()
...
class TestContextToken:
    ...
    async def test_create_token(self, get_fakeUOW: FakeUOWContext, create_data_token):
        async with get_fakeUOW as uow:
    ...
    async def test_update_token(self, get_fakeUOW: FakeUOWContext, update_data_token):
        async with get_fakeUOW as uow:
            token = await uow.token.update(
                jti=self.data_jti["jti"], obj_in=update_data_token
            )
            assert token
